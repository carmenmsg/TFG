...
//	}
	...
	public Map<String, List<Sequence>> getNombreMap() {
		return this.nombreMap;
	}
	
	public void actualizarNombreMap(){
		this.nombreMap.clear();		//Esto es necesario para que no agregue solo. Al borrarlo no habrá partes repes.
		for(String c : this.getClases()){
			if (!nombreMap.containsKey(c))
		    	nombreMap.put(c, new LinkedList<Sequence>());
		}
		for(int i =0; i < this.getSequences().size(); i++) {
			nombreMap.get(this.clases.get(i)).add(this.getSequences().get(i));
		}
	}
