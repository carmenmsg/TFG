...
public class Sequence {
	 ...
	 */
	public Sequence cloneSequenceMinusItems(Map<Integer, Map<String,Set<Integer>>> mapSequenceID, double relativeMinSup) {
	 ...
	 * @return a copy of this itemset except that item(s) with a support lower than minsup have been excluded.
	 */
	public List<Integer> cloneItemsetMinusItems(List<Integer> itemset, Map<Integer, Map<String, Set<Integer>>> mapSequenceID, double minSupportAbsolute) {
		// create a new itemset
		List<Integer> newItemset = new ArrayList<Integer>();
		// for each item of the original itemset
		for(Integer item : itemset){
			for(String c : mapSequenceID.get(item).keySet()){
				// get the sed of sequences containing this item
				Set<Integer> sidSet = mapSequenceID.get(item).get(c);
