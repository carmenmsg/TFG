 ...
 */
@AuthoredType(typeName = "BIDE Sequential Patterns",
...
public class SequentialPatterns {
	...
	public void printFrequentPatterns(int nbObject, boolean showSequenceIdentifiers){
	  sizeSequenceDatabase = nbObject;
		System.out.println(toString(nbObject,showSequenceIdentifiers));
	}
	
	/**
	 * Yop
	 * @return sizeSequenceDatabase (tamanio de database)
	 */
	public int getSizeSequenceDatabase() {
	  return sizeSequenceDatabase;
	}
	...
	public String toString(int nbObject, boolean showSequenceIdentifiers){
		...
		for(List<SequentialPattern> level : levels){
			...
			for(SequentialPattern sequence : level){
				...
				r.append(")" );
				r.append(" -- ");
				r.append(sequence.getIG());
				if(showSequenceIdentifiers) {
					for(String clase : sequence.getSequenceIDs().keySet()){
						r.append("clase: ");
						r.append(clase);	
			        	r.append(" sequence ids: ");
			        	for (Integer sid: sequence.getSequenceIDs().get(clase)) { // TODO Check if it is necessary to print also each class of the sequenceIDs Sets
			        		r.append(sid);
			        		r.append(" ");
			        	}
					}
