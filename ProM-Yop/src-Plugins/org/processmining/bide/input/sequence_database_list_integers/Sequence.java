...
import org.processmining.bide.input.sequence_database_array_integers.SequenceDatabase;
...
public class Sequence {
	 ...
	 */
	public Sequence cloneSequenceMinusItems(Map<Integer, Map.Entry<Map<String,Set<Integer>>,Integer>> mapSequenceID, double relativeMinSup) {
	 ...
	 * @return a copy of this itemset except that item(s) with a support lower than minsup have been excluded.
	 */
	public List<Integer> cloneItemsetMinusItems(List<Integer> itemset, Map<Integer, Map.Entry<Map<String, Set<Integer>>,Integer>> mapSequenceID, double minSupportAbsolute) {
		// create a new itemset
		List<Integer> newItemset = new ArrayList<Integer>();
		// for each item of the original itemset
		for(Integer item : itemset){
			// get the sed of sequences containing this item
			Map<String,Set<Integer>> sidSet = mapSequenceID.get(item).getKey();
			// if this set is not null (an infrequent item) and the support is higher than minsup...
			if(sidSet != null && mapSequenceID.get(item).getValue() >= minSupportAbsolute){
				newItemset.add(item); // add it to the new itemset
			}
		}
		return newItemset; // return the new itemset.
